...
|`location`|String|Required|The location where you want to deploy your instance. The location must match the `region` parameter that you specify in the `provider` block of your Terraform configuraiton file. |
...
|`adminpassword`|String|Optional| The password for the database administrator. If not specified, the password is unitialised and the user ID cannot be used. In this case, addditional users must be specified in a `user` block.|
