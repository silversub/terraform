...
If you want to create, update, or delete Power System resources in a multizone-capable region, you must specify the `zone` in the `provider` block of your Terraform on {{site.data.keyword.cloud_notm}} configuration file. For more information, see the [`provider` block configuration](/docs/terraform?topic=terraform-provider-reference).
{: important}
